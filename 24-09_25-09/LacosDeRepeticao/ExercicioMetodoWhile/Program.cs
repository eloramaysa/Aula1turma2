...
namespace ExercicioMetodoWhile
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Informe a palavra de busca com 3 caracteres: ");
            var wordCount = FindByWhile(Console.ReadLine());

            Console.WriteLine($"Encontrador: {wordCount}");
            Console.ReadKey();
            
        }
        /// <summary>
        /// Metodo para buscar texto dentro da aplicação
        /// </summary>
        /// <param name="wordFind">Parametro para realizar a busca</param>
        /// <returns>Retorna a quantidade encontrada </returns>
        private static int FindByWhile (string wordFind)
        {
            var textoProcura = TextContent();
            var wordFindCountLength = wordFind.Length;
            //-1 porque o Length nao conta a partir do 0 e sim a partir do 1
            var lengthTexto = textoProcura.Length - (wordFindCountLength -1);
            //ctrl + R + R muda o nome da variavel em todos os locais em que ela esta 
            var i = 0;
            var contador = 0;
                while (i<lengthTexto)
            {
                ...
                {
                    compareWord += textoProcura[i + b].ToString();
                    b++;
                }
                if (compareWord == wordFind) contador++;
                i++;
            }
            return contador;
        }
        /// <summary>
        /// Metodo para retornar um texto grande para analisar
        /// </summary>
        /// <returns></returns>
